...
public partial class Mantenimientos_Seguridad_MantUsuario : BaseMantListado
{
    #region Declaraciones
    private string PaginaDetalle = "MantUsuarioDetalle.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            this.Master.TituloModulo = "Mantenimiento de Usuarios";
            #region accesos
            Accesos();
            #endregion accesos
    ...
    protected void btnModificar_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            if (gv.SelectedIndex != -1)
            {
                ...
                string CodUsuario = "&IdUsuario=" + gv.SelectedRow.Cells[1].Text.ToString();
                Response.Redirect(PaginaDetalle + str_estado + CodUsuario);
            }
            else
            {
    ...
    protected override void Obtener_Datos()
    {
        ...
        objEnUsuario.Accion = G_Accion.ToString();
        objEnUsuario.login3 = Util.FormateaTexto(txt_login3.Text.Trim());
        objEnUsuario.nombreusuario = Util.FormateaTexto(txt_NOMBRE.Text.Trim());
        objEnUsuario.CEmpresa = (String)this.Session["cempresa"];
        objEnUsuario.cod_jerarquiaA = cmb_Jerarquia.SelectedValue.ToString().Trim();

        ListEnUsuario.Add(objEnUsuario);
        try
        {
            //WCF
            DT_Datos = objLoUsuario.Listado_Usuario(ListEnUsuario);
    ...
    protected override void Ejecutar_Busqueda()
    {
        try
        {
            ...
            {
                if (txt_NOMBRE.Text.Length < 1 && txt_login3.Text.Length < 1)
                {
                    return;
                }
    ...
    private void Exportar(string Formato)
    {
        try
        {
            ...
            string str_nombre = "&nomusuario=" + Util.FormateaTexto(txt_NOMBRE.Text.ToUpper().Trim());
            string str_login3 = "&login3=" + Util.FormateaTexto(txt_login3.Text.Trim());
            string str_cempresa = "&cempresa=" + (String)this.Session["cempresa"];
            string str_TIPOCONSULTA = "&tipoconsulta=" + "0";

            str_Parametros = str_tiporeporte + str_accionstore + str_nombre + str_login3 + str_cempresa + str_TIPOCONSULTA;
            ...
            sb.Append("<script>var win=window.open('../../Reportes/Seguridad/ReporteUsuarioC.aspx" + str_Parametros + "', 'ReportePersonal', " + CONFIG + ");</script>");
    ...
    private void Anular()
    {
        //MostrarMensaje("No se Puede Anular el Registro.", true);
        ...
        try
        {
            ...
            ObjEnUsuario.id = gv.SelectedRow.Cells[1].Text.ToString();
            ObjEnUsuario.CEmpresa = (String)this.Session["cempresa"];
            ...
            msg = ObjLoUsuario.Anula_Usuario(ListEnUsuario);
    ...
    protected void cmb_Jerarquia_SelectedIndexChanged(object sender, EventArgs e)
    {
        G_Accion = AccionListado.Filtrado;
        RefrescarGrid();
    }
