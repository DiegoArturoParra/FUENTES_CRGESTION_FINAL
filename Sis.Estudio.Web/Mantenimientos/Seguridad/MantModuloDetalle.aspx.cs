...
public partial class Mantenimientos_Seguridad_MantModuloDetalle : BaseMantDetalle
{
    #region Declaraciones
    private const string PaginaRetorno = "MantModulo.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            this.Master.TituloModulo = "Detalle de Modulo";
            #region accesos
            Accesos();
            #endregion accesos
    ...
    protected void MostrarDatos(string str_TIPO, string str_ID)
    {
        ...
        try
        {
            ...
            ObjEnModulo.Tipo = str_TIPO;
            ObjEnModulo.Id =str_ID;
            ListEnModulo.Add(ObjEnModulo);
            dt = ObjLoModulo.CargaDatosModulo(ListEnModulo);
            if (dt.Rows.Count > 0)
            {
                #region CONTROLES_MANTENIMIENTO
                txt_IDPERFIL.Text = dt.Rows[0]["IdModulo"].ToString();
                ...
                txt_NOMIMAGEN.Text = dt.Rows[0]["imagen"].ToString();

                #region Slanzador
                string SLanzador = dt.Rows[0]["SLanzador"].ToString();
                if (SLanzador == "S")
                {
                    chk_LANZADOR.Checked = true;
                }
                else
                {
                    chk_LANZADOR.Checked = false;
                }
                #endregion Slanzador
    ...
    private void EnableControl(bool dato)
    {
        ...
        txt_URL.Enabled = dato;
        txt_NOMIMAGEN.Enabled = dato;
        chk_LANZADOR.Enabled = dato;
    ...
    protected void LimpiarControles()
    {
        ...
        txt_NOMIMAGEN.Text = String.Empty;
        chk_LANZADOR.Checked = false;
    ...
    private void Grabar()
    {
        ...
        try
        {
            ...
            EnModulo ObjEnModulo = new EnModulo();
            List<EnModulo> ListEnModulo = new List<EnModulo>();
            ObjEnModulo.CodSistema = FlagsPrograma.FLG_VALORSISTEMA;
            ObjEnModulo.CEmpresa = (String)this.Session["cempresa"];
            ObjEnModulo.Nombre = txt_NOMBRE.Text.Trim();
            ObjEnModulo.Descripcion = txt_DESCRIPCION.Text.Trim();
            ObjEnModulo.Url = txt_URL.Text.Trim();
            ObjEnModulo.Imagen = txt_NOMIMAGEN.Text.Trim();
            #region SLanzador
            string SLanzador  = "N";
            if (chk_LANZADOR.Checked == true)
            {
                SLanzador = "S";
            }
            ObjEnModulo.SLanzador = SLanzador;
            #endregion SLanzador
            ObjEnModulo.CodUsuario =  (String)this.Session["codusuario"];
            ObjEnModulo.Id = txt_IDPERFIL.Text.Trim();
            ListEnModulo.Add(ObjEnModulo);
            #endregion Cargar_Variables
            List<EnTransaccion> RetornoT = ObjLoModulo.Insertar_Modulo(ListEnModulo);
    ...
    private void Modificar()
    {
        ...
        try
        {
            ...
            ListEnModulo.Add(ObjEnModulo);
            #endregion Cargar_Variables                       
            msg = ObjLoModulo.Modifica_Modulo(ListEnModulo);
            if (msg == "") { Exito = FlagsPrograma.FLG_VALOREXITOSI; } else { Master.MostrarMensaje(msg, TipoMensaje.Error, msg); Exito = FlagsPrograma.FLG_VALOREXITONO; return; }
        }
        catch (SqlException ex)
        {
    ...
    private void Anular()
    {
        ...
        try
        {
            ...
            msg = ObjLoModulo.Anula_Modulo(ListEnModulo);
