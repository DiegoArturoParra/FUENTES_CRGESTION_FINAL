...
public partial class Estudio_Gestion_GS_EjecutadoDetalle : BaseMantDetalle
{
    #region Declaraciones
    private const string PaginaRetorno = "GS_Ejecutado.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            G_idopcion = OpcionModulo.MantEjecutado;
            this.Master.TituloModulo = "Detalle de Clasificación de Action Plan";
            #region accesos
            Accesos();
            #endregion accesos
    ...
    protected void MostrarDatos(string str_TIPO, string str_ID)
    {
        ...
        try
        {
            ...
            ObjEnGS_Ejecutado.CodEjecutado = str_ID;
            ListEnGS_Ejecutado.Add(ObjEnGS_Ejecutado);
            dt = ObjLoGS_Ejecutado.GS_Ejecutado_Reg(ListEnGS_Ejecutado);
            if (dt.Rows.Count > 0)
            {
                #region CONTROLES_MANTENIMIENTO
                txt_codigo.Text = dt.Rows[0]["CodEjecutado"].ToString();
    ...
    private bool Valida_Datos()
    {
        ...
        if (txt_tiempo.Text == "")
        {
            MostrarMensaje("Ingrese el tiempo.", true);
            Cursor_Control(txt_tiempo);
            return false;
        }

        if (txt_dias.Text == "")
        {
            MostrarMensaje("Ingrese los dias.", true);
            Cursor_Control(txt_dias);
            return false;
        }
    ...
    private void Grabar()
    {
        ...
        try
        {
            ...
            ObjEnGS_Ejecutado.Descripcion = txt_descripcion.Text.Trim();
            ObjEnGS_Ejecutado.Tiempo = txt_tiempo.Text.Trim();
            ObjEnGS_Ejecutado.dias = txt_dias.Text.Trim();
            ObjEnGS_Ejecutado.CodTipoGestion = cmb_CodTipoGestion.SelectedValue.ToString();
            ObjEnGS_Ejecutado.CodUsuario = (String)this.Session["codusuario"];


            ListEnGS_Ejecutado.Add(ObjEnGS_Ejecutado);
            #endregion Cargar_Variables
            List<EnTransaccion> RetornoT = ObjLoGS_Ejecutado.GS_Ejecutado_INS(ListEnGS_Ejecutado);
    ...
    private void Modificar()
    {
        ...
        try
        {
            ...
            ObjEnGS_Ejecutado.CodEjecutado = txt_codigo.Text.Trim();
            ObjEnGS_Ejecutado.Descripcion = txt_descripcion.Text.Trim();
            ObjEnGS_Ejecutado.Tiempo = txt_tiempo.Text.Trim();
            ObjEnGS_Ejecutado.dias = txt_dias.Text.Trim();
            ObjEnGS_Ejecutado.CodTipoGestion = cmb_CodTipoGestion.SelectedValue.ToString();
            ...
            ListEnGS_Ejecutado.Add(ObjEnGS_Ejecutado);
            #endregion Cargar_Variables                       
            msg = ObjLoGS_Ejecutado.GS_Ejecutado_UPD(ListEnGS_Ejecutado);
            if (msg == "exito") { Exito = FlagsPrograma.FLG_VALOREXITOSI; } else { Master.MostrarMensaje(msg, TipoMensaje.Error, msg); Exito = FlagsPrograma.FLG_VALOREXITONO; return; }
        }
        catch (SqlException ex)
        {
    ...
    private void Anular()
    {
        ...
        try
        {
            ...
            msg = ObjLoGS_Ejecutado.GS_Ejecutado_DEL(ListEnGS_Ejecutado);
