...
public partial class Estudio_Gestion_GS_ReglasGestiones : BaseMantListado
{
    #region Declaraciones
    private string PaginaDetalle = "GS_ReglasGestionesDetalle.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            this.Master.TituloModulo = "Mantenimiento de Reglas de Gestiones";
            #region accesos
            Accesos();
            #endregion accesos
            CargaComboEmpresa();
    ...
    protected override void Obtener_Datos()
    {
        ...
        objEnGS_ReglasGestiones.nempresa = cmb_Empresa.SelectedValue.ToString().Trim();
        objEnGS_ReglasGestiones.Tramo = cmb_Tramo.SelectedValue.ToString().Trim();
      
        objEnGS_ReglasGestiones.descripcion = Util.FormateaTexto(txt_DESCRIPCION.Text.Trim());
        ListEnGS_ReglasGestiones.Add(objEnGS_ReglasGestiones);
        
        try
        {
            //WCF
            if (cmb_Tramo.SelectedValue.ToString().Trim() != "-1")
                DT_Datos = objLoGS_ReglasGestiones.GS_ReglasGestiones_Lista(ListEnGS_ReglasGestiones);
            else
                DT_Datos = objLoGS_ReglasGestiones.GS_ReglasGestiones_ListaTodos(ListEnGS_ReglasGestiones);
        }
        catch (Exception ex)
        {
            throw ex;
    ...
    private void Anular()
    {
        ...
        try
        {
            ...
            ObjEnGS_ReglasGestiones.id_ReglasGestiones = gv.SelectedRow.Cells[1].Text.ToString();
