...
public partial class Estudio_Maestros_CalificacionSBSDet : BaseMantDetalle
{
    #region Declaraciones
    private const string PaginaRetorno = "CalificacionSBS.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            G_idopcion = OpcionModulo.CalificacionSBS;
            this.Master.TituloModulo = "Detalle de Calificacion SBS";
    ...
    protected void MostrarDatos(string str_TIPO, string str_ID)
    {
        ...
        try
        {
            ...
            List<EnCalificacionSBS> objListEn = new List<EnCalificacionSBS>();
            EnCalificacionSBS objEn = new EnCalificacionSBS();

            objEn.NEmpresa = (String)this.Session[Global.NEmpresa].ToString();
            objEn.CodCalificacionSBS = str_ID;
            ...
            dt = objLo.CalificacionSBS_Listar_Reg(objListEn);
            if (dt.Rows.Count > 0)
            {
                #region CONTROLES_MANTENIMIENTO
                txt_ID.Text = dt.Rows[0]["CodCalificacionSBS"].ToString();
                txt_NOMBRE.Text = dt.Rows[0]["CalificacionSBS"].ToString();
                txt_IniDiasMora.Text = dt.Rows[0]["IniDiasMora"].ToString();
                txt_FinDiasMora.Text = dt.Rows[0]["FinDiasMora"].ToString();
    ...
    private void EnableControl(bool dato)
    {
        ...
        txt_IniDiasMora.Enabled = dato;
        txt_FinDiasMora.Enabled = dato;
    ...
    protected void LimpiarControles()
    {
        ...
        txt_IniDiasMora.Text = String.Empty;
        txt_FinDiasMora.Text = String.Empty;
    ...
    private bool Valida_Datos()
    {
        ...
        if (txt_NOMBRE.Text == "")
        {
            MostrarMensaje("Ingrese Calificación.", true);
            Cursor_Control(txt_NOMBRE);
            return false;
        }

        if (txt_IniDiasMora.Text.Trim().Length > 0)
        {
            if (Util.esNumero(txt_IniDiasMora.Text) == false)
            {
                MostrarMensaje("Ingrese Valores Numericos.", true);
                Cursor_Control(txt_IniDiasMora);
                return false;
            }        
        }


        if (txt_FinDiasMora.Text.Trim().Length > 0)
        {
            if (Util.esNumero(txt_FinDiasMora.Text) == false)
            {
                MostrarMensaje("Ingrese Valores Numericos.", true);
                Cursor_Control(txt_FinDiasMora);
                return false;
            }
        }
    ...
    private void Grabar()
    {
        ...
        try
        {
            ...
            objEn.NEmpresa = (String)this.Session[Global.NEmpresa].ToString();
            objEn.CodCalificacionSBS = Util.FormateaEntero(txt_ID.Text);
            objEn.CalificacionSBS = txt_NOMBRE.Text;                         
            objEn.IniDiasMora = Util.FormateaEntero(txt_IniDiasMora.Text);
            objEn.FinDiasMora = Util.FormateaEntero(txt_FinDiasMora.Text);
            ...
            List<EnTransaccion> RetornoT = ObjLo.CalificacionSBS_INS(objListEn);
    ...
    private void Modificar()
    {
        try
        {
            ...
            #endregion Carga_Variable
            LoCalificacionSBS ObjLo = new LoCalificacionSBS();
            msg = ObjLo.CalificacionSBS_UPD(objListEn);
            if (msg == "exito")
            {
                metodo_Consulta();
