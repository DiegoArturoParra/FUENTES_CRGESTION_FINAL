...
public partial class Estudio_Maestros_ZonaDet : BaseMantDetalle
{
    #region Declaraciones
    private const string PaginaRetorno = "Zona.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            G_idopcion = OpcionModulo.Zona;
            this.Master.TituloModulo = "Detalle de Zona";
    ...
    protected void MostrarDatos(string str_TIPO, string str_ID)
    {
        ...
        try
        {
            ...
            List<EnZona> objListEn = new List<EnZona>();
            EnZona objEn = new EnZona();

            objEn.NEmpresa = (String)this.Session[Global.NEmpresa].ToString();
            objEn.CodZona = str_ID;
            ...
            dt = objLo.Zona_Listar_Reg(objListEn);
            if (dt.Rows.Count > 0)
            {
                #region CONTROLES_MANTENIMIENTO
                txt_ID.Text = dt.Rows[0]["codzona"].ToString();
                cmb_Gerencia.SelectedValue = dt.Rows[0]["CodGerencia"].ToString();
                txt_NOMBRE.Text = dt.Rows[0]["Zona"].ToString();
    ...
    private bool Valida_Datos()
    {
        ...
        {
            MostrarMensaje("Seleccion Gerencia", true);
            return false;
        }

        if (txt_NOMBRE.Text == "")
        {
            MostrarMensaje("Ingrese Zona.", true);
            Cursor_Control(txt_NOMBRE);
            return false;
        }
    ...
    private void Grabar()
    {
        ...
        try
        {
            ...
            objEn.CodZona = Util.FormateaEntero(txt_ID.Text);                       
            objEn.Zona = txt_NOMBRE.Text;   
            ...
            List<EnTransaccion> RetornoT = ObjLo.Zona_INS(objListEn);
