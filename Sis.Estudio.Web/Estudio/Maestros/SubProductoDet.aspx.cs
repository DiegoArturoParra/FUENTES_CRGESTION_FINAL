...
public partial class Estudio_Maestros_SubProductoDet : BaseMantDetalle
{
    #region Declaraciones
    private const string PaginaRetorno = "SubProducto.aspx";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            G_idopcion = OpcionModulo.SubProducto;
            this.Master.TituloModulo = "Detalle de Sub Producto";
    ...
    protected void MostrarDatos(string str_TIPO, string str_ID)
    {
        ...
        try
        {
            ...
            List<EnSubProducto> objListEn = new List<EnSubProducto>();
            EnSubProducto objEn = new EnSubProducto();

            objEn.NEmpresa = (String)this.Session[Global.NEmpresa].ToString();
            objEn.CodSubProducto = str_ID;
            ...
            dt = objLo.SubProducto_Listar_Reg(objListEn);
            if (dt.Rows.Count > 0)
            {
                #region CONTROLES_MANTENIMIENTO
                txt_ID.Text = dt.Rows[0]["CodSubProducto"].ToString();
                cmb_Producto.SelectedValue = dt.Rows[0]["CodProducto"].ToString();
                txt_NOMBRE.Text = dt.Rows[0]["SubProducto"].ToString();
    ...
    private bool Valida_Datos()
    {
        ...
        if (cmb_Producto.Items.Count < 1)
        {
            MostrarMensaje("Seleccion Producto", true);            
            return false;
        }
        ...
        if (txt_NOMBRE.Text == "")
        {
            MostrarMensaje("Ingrese Sub Producto.", true);
            Cursor_Control(txt_NOMBRE);
            return false;
        }
    ...
    private void Grabar()
    {
        ...
        try
        {
            ...
            objEn.NEmpresa = (String)this.Session[Global.NEmpresa].ToString();
            objEn.CodSubProducto = Util.FormateaEntero(txt_ID.Text);
            objEn.SubProducto = txt_NOMBRE.Text;
            ...
            List<EnTransaccion> RetornoT = ObjLo.SubProducto_INS(objListEn);
    ...
    private void Modificar()
    {
        try
        {
            ...
            #endregion Carga_Variable
            LoSubProducto ObjLo = new LoSubProducto();
            msg = ObjLo.SubProducto_UPD(objListEn);
            if (msg == "exito")
            {
                metodo_Consulta();
